...
def win():
    if user_score == [0] or sum(computer_cards) > 21:
        print(f'{user_score}Game over. You win!')
        exit()
    elif computer_score == [0] or sum(user_cards) > 21:
        print(f'{user_score}Game over. You lose.')
        exit()
...
def calculate_score():
    ...
    computer_score.append(sum(computer_cards))
    is_blackjack()
...
game_continue = input('Do you want to hit? ')
...
while sum(user_cards) < 21:
    print(user_score)
    game_continues = input('Do you want to hit? ')
    game_continue()
